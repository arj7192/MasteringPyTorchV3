# Email update : Aug 2026

**To:** Shashank Desai <shashankd@packt.com>
**Cc:** Tiksha Abhimanyu Lad <tikshas@packt.com>; Sanjana Gupta <sanjanag@packt.com>
**Subject:** Re: Title Update - Mastering Pytorch, 3E

---

## Gmail-ready body (copy below)

```
Hi Shashank,

Sorry for the long silence since my June 28 note, and for missing the July 3 update I said I'd send. That was on me.

Quick status, and no change to the timeline from my side.

What's done (companion repo: https://github.com/arj7192/MasteringPyTorchV3):
- Outline, change log, and plan locked to the May 20 decisions (Responsible AI woven into the chapters, NST folded into GANs, Llama 3 as the main open-weight example)
- Full 3E chapter folders remapped from 2E across all four waves (19 chapters). Carry-over notebooks are in place for Waves 1–4; Ch 5 multimodal and Ch 8 fine-tuning LLMs are scaffolded as net-new (README only for now)
- Planned removals applied: poutyne, RandWireNN, GPT-3 OpenAI notebook; no standalone NST / CNN+LSTM chapters
- Deps floored to current PyTorch 2.x (torch 2.13); torchvision weights API updated where needed
- Modernization furthest along on Wave 1: Ch 1 torch.compile demo notebook, Ch 1 prose drafts for the PyTorch 2.x section and Responsible AI primer, Lightning notebook API fixes, notebook validation scripts. Later-wave 3E upgrades (Llama 3, SDXL, FSDP, vLLM, etc.) still to write on top of the remapped base
- Live status board at _planning/STATUS.md

How I'm writing this edition (same pattern as 2E): I start with the code and notebooks, then diagrams, then the actual chapter copy. That way the manuscript tracks something that actually runs. The remapped codebase above is that first stage for all waves. I'm on the chapter manuscripts now, starting with Wave 1 (Ch 1, 2, 3, 15, 17).

Still to do before those first drafts are with you:
- Finish Wave 1 code cleanups that block the text (e.g. torchtext → HF Datasets on the RNN chapter)
- Diagrams for the refreshed / new sections
- Turn the Ch 1 drafts and the rest of Wave 1 into Packt-ready chapter copy
- Then the same loop for Waves 2–4 (3E code upgrades on the remapped base, diagrams, then manuscript: heavy refactors, then net-new chapters: multimodal, fine-tuning LLMs, Llama 3 / MusicGen, SDXL / ControlNet, RLHF, production / edge)

I'll send the next update with Wave 1 chapter drafts as they land. Happy to jump on a short call this week if useful.

Best,
Ashish

--
Ashish Ranjan Jha
Co-Founder & CEO, Nativ
```
